...
class Generator{
  constructor () {
    this.NN = tf.model();
    this.NN.add(tf.layers.dense({units: 10, inputShape: [SEED_SIZE]}));
    
  }
}

// Network arch for generator
let G1w = varInitNormal([SEED_SIZE, 500]);
let G1b = varInitNormal([500]);
let G2w = varInitNormal([500, 300]);
let G2b = varInitNormal([300]);
let G3w = varInitNormal([300, INPUT_SIZE]);
let G3b = varInitNormal([INPUT_SIZE]);

// Network arch for discriminator
let D1w = varInitNormal([INPUT_SIZE, 600]);
let D1b = varInitNormal([600]);
let D2w = varInitNormal([600, 400]);
let D2b = varInitNormal([400]);
let D3w = varInitNormal([400, 1]);
